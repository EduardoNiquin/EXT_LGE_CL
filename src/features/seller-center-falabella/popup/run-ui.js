// UI de progreso para "SoporteSeller — Detalle Orden". Reusa las clases CSS
// .lt-* / .ct-* ya definidas en popup.css. El run vive en chrome.storage.local;
// la UI se actualiza vía storage.onChanged.
...
export function progressHtml() {
  return `
    <section id="scf-progress" class="lt-progress hidden">
      <div class="lt-progress-head">
        <strong id="scf-progress-title">Procesando…</strong>
        <span id="scf-progress-counter" class="dt-progress-counter"></span>
      </div>
      <div id="scf-progress-bar" class="lt-progress-bar"><span></span></div>
      <ul id="scf-item-list" class="lt-region-list"></ul>
      <details class="ct-diag lt-log-details">
        <summary>Registro</summary>
        <ul id="scf-log" class="lt-log"></ul>
      </details>
    </section>
  `;
}
...
export function wireRunControls(container) {
  container.querySelector('#scf-stop')?.addEventListener('click', onStop);
  container.querySelector('#scf-clear')?.addEventListener('click', () => onClear(container));
...
export function hideProgress(container) {
  container.querySelector('#scf-progress')?.classList.add('hidden');
}
...
export function toggleButtons(container, run) {
  ...
  const stopBtn  = container.querySelector('#scf-stop');
  const clearBtn = container.querySelector('#scf-clear');
  if (startBtn) startBtn.disabled = active;
  if (stopBtn)  stopBtn.disabled  = !active;
  if (clearBtn) clearBtn.classList.toggle('hidden', !finished);
  container.querySelectorAll('.lt-form-card input, .lt-form-card textarea, .lt-form-card select, .lt-form-card button.scf-mode-btn')
    .forEach((el) => { el.disabled = active; });
}

export function renderProgress(container, run) {
  const wrap = container.querySelector('#scf-progress');
  if (!wrap) return;
  if (!run) { wrap.classList.add('hidden'); return; }
  wrap.classList.remove('hidden');

  const stats = computeStats(run);
  const titleEl = container.querySelector('#scf-progress-title');
  if (run.active) titleEl.textContent = 'Procesando…';
  else if (run.finishReason === 'cancelled') titleEl.textContent = 'Cancelado';
  else if (run.finishReason === 'not-detected') titleEl.textContent = 'Formulario no detectado';
  else if (run.finishReason === 'error') titleEl.textContent = 'Finalizado con errores';
  else titleEl.textContent = 'Finalizado';

  const counterEl = container.querySelector('#scf-progress-counter');
  const breakdown = [];
  ...
  const bar = container.querySelector('#scf-progress-bar span');
  if (bar) bar.style.width = `${pct}%`;

  const itemList = container.querySelector('#scf-item-list');
  itemList.innerHTML = '';
  (run.items || []).forEach((item, idx) => {
    ...
    li.innerHTML = `
      <div class="lt-region-head">
        <span class="lt-region-name">#${idx + 1} · orden ${escapeHtml(item.ordernumber)}</span>
        <span class="lt-region-leadtimes">guía ${escapeHtml(item.guia)} · ${escapeHtml(item.cantP)} paq.</span>
  ...
  const logEl = container.querySelector('#scf-log');
  logEl.innerHTML = '';
