...
//
// Defensivo: si un campo no existe se omite; grupos sin campos no se devuelven.
...
export function extractPbpProduct(response) {
  ...
  groups.push(group('precios', 'Precios', [
    field('Precio final', finalPrice.value, formatCLP(finalPrice.value)),
    field('Precio regular', regular.value ?? product.msrp_price, formatCLP(regular.value ?? product.msrp_price)),
    field('MSRP', product.msrp_price, formatCLP(product.msrp_price)),
    field('Descuento ($)', discount.amount_off, formatCLP(discount.amount_off)),
    field('Descuento (%)', discount.percent_off, formatPercent(discount.percent_off)),
    field('Moneda', regular.currency),
  ...
  // 4. Despacho (+ cobertura por comuna si vino)
  ...
  // 10. Instalación (servicio de instalación, suele venir vacío)
  ...
  // 11. Marketing (textos del paquete principal)
