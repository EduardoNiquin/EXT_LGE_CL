// UI de progreso para "Verificar órdenes y stock". Reusa las clases CSS
// .lt-* / .ct-* / .dt-* ya definidas en popup.css. El run vive en
...
export function progressHtml() {
  return `
    <section id="sk-progress" class="lt-progress hidden">
      <div class="lt-progress-head">
        <strong id="sk-progress-title">Procesando…</strong>
        <span id="sk-progress-counter" class="dt-progress-counter"></span>
      </div>
      <div id="sk-progress-bar" class="lt-progress-bar"><span></span></div>
      <ul id="sk-item-list" class="lt-region-list"></ul>
      <details class="ct-diag lt-log-details">
        <summary>Registro</summary>
        <ul id="sk-log" class="lt-log"></ul>
      </details>
    </section>
  `;
}
...
export function wireRunControls(container) {
  container.querySelector('#sk-stop')?.addEventListener('click', onStop);
  container.querySelector('#sk-clear')?.addEventListener('click', () => onClear(container));
...
export function hideProgress(container) {
  container.querySelector('#sk-progress')?.classList.add('hidden');
}

export function toggleButtons(container, run) {
  const active   = Boolean(run?.active);
  const finished = Boolean(run && !run.active);
  const startBtn = container.querySelector('#sk-start');
  const stopBtn  = container.querySelector('#sk-stop');
  const clearBtn = container.querySelector('#sk-clear');
...
export function renderProgress(container, run) {
  const wrap = container.querySelector('#sk-progress');
  if (!wrap) return;
  if (!run) { wrap.classList.add('hidden'); return; }
  wrap.classList.remove('hidden');

  const stats = computeStats(run);
  const titleEl = container.querySelector('#sk-progress-title');
  if (run.active) titleEl.textContent = 'Procesando…';
  else if (run.finishReason === 'cancelled') titleEl.textContent = 'Cancelado';
  else if (run.finishReason === 'not-detected') titleEl.textContent = 'Pestaña no detectada';
  else if (run.finishReason === 'error') titleEl.textContent = 'Finalizado con errores';
  else titleEl.textContent = 'Finalizado';

  const counterEl = container.querySelector('#sk-progress-counter');
  const breakdown = [];
  if (stats.ok       > 0) breakdown.push(`<span class="lt-stat-ok">${stats.ok} ok</span>`);
  if (stats.skipped  > 0) breakdown.push(`<span class="lt-stat-skipped">${stats.skipped} omit.</span>`);
  if (stats.notFound > 0) breakdown.push(`<span class="lt-stat-skipped">${stats.notFound} sin producto</span>`);
  ...
  const bar = container.querySelector('#sk-progress-bar span');
  if (bar) bar.style.width = `${pct}%`;

  const itemList = container.querySelector('#sk-item-list');
  itemList.innerHTML = '';
  (run.items || []).forEach((item, idx) => {
    ...
    li.innerHTML = `
      <div class="lt-region-head">
        <span class="lt-region-name">#${escapeHtml(item.orderNumber)}${item.reference ? ` · ${escapeHtml(item.reference)}` : ''}</span>
  ...
  const logEl = container.querySelector('#sk-log');
  logEl.innerHTML = '';
...
export function computeStats(run) {
  ...
  let ok = 0; let error = 0; let notFound = 0; let skipped = 0;
  for (const it of items) {
    if      (it.status === STATUS.OK)        ok++;
    else if (it.status === STATUS.ERROR)     error++;
    else if (it.status === STATUS.NOT_FOUND) notFound++;
    else if (it.status === STATUS.SKIPPED)   skipped++;
  }
  return { total: items.length, done: ok + error + notFound + skipped, ok, error, notFound, skipped };
}
...
function itemStatusClass(s) {
  switch (s) {
    ...
    case STATUS.NOT_FOUND: return 'error';
...
function labelStatus(s) {
  switch (s) {
    ...
    case STATUS.ERROR:     return 'Error';
    case STATUS.NOT_FOUND: return 'Falta producto';
    case STATUS.SKIPPED:   return 'Omitida';
    default: return s || '';
  }
}
...
function detailForItem(item) {
  ...
  const stocked = ps.filter((p) => p.action === 'stocked' || p.action === 'would-stock').length;
  const hasStock = ps.filter((p) => p.action === 'has-stock').length;
  const parts = [];
  if (stocked)  parts.push(`${stocked} c/stock asignado`);
  if (hasStock) parts.push(`${hasStock} ya tenía`);
  return parts.join(' · ');
}
