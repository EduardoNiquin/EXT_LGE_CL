...
**API (`background/informe.js`):** `GET https://147.93.176.66/api/magento/detalle-ordenes?from=YYYY-MM-DD&to=YYYY-MM-DD` con header `X-Api-Token` (token `MAGENTO_PA_TOKEN`, hardcodeado en `constants.js#API`). Vive en el repo **`obs`** (`app/Http/Controllers/Api/DetalleOrdenesMagentoController.php`, test `tests/Feature/ObsPortal/DetalleOrdenes/Magento/ApiLecturaTest.php`) y lee el **detalle de ordenes del portal** (`detalle_ordenes` + `detalle_ordenes_crudo`, `canal = 'obs'`), donde llega tanto Power Automate como el CSV que se sube en *Configuracion → Ordenes*. Responde `{count, hasta, data:[...]}`:
- el rango va sobre `detalle_ordenes.fecha` (**ya en hora de Chile**, ambos extremos inclusive): no hay desfase ni ventana ancha; `processReport` vuelve a filtrar igual.
- cada fila trae **solo las 14 columnas de la lista blanca** del server (`CAMPOS`, las mismas de `OUTPUT_COLUMNS`) con las cabeceras del CSV de Magento; `Local Time` sale de `fecha` (siempre `YYYY-MM-DD HH:MM:SS`). Si el informe necesita una columna nueva, hay que sumarla **en los dos repos**.
- `hasta` = la orden mas nueva del canal: con 0 filas el error dice "el portal tiene ordenes solo hasta X" y, si el rango queda cubierto solo en parte, el log avisa "Rango incompleto".

**Historia:** hasta sep-2026 se usaba `GET /api/magento/orders` (tabla vieja `MagentoOrders_PowerAutomate`, filtraba por `order_date` en UTC con ventana +-1 dia). Esa tabla dejo de llenarse el 2026-09-16 (lo nuevo se sube por el portal y solo llega a `detalle_ordenes*`), por eso rangos posteriores devolvian 0 filas. Cancelacion via `AbortController` + mensaje `e-promoters:informe:cancel`.
