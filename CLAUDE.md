...
`logger('foo')` registra el scope `foo`, con toggle individual en Ajustes (`features/ajustes`) + "Habilitar/Deshabilitar todos". `log-config` cachea en memoria y persiste en `chrome.storage.local` (`log-config:scopes`, cross-context vía `storage.onChanged`); `logger.js` chequea `isScopeEnabled(scope)` antes de emitir. Default: todos habilitados. Hay un scope por feature/módulo (`colocar-tags[:product|:offer|:delivery-remove|:combobox]`, `magento/<módulo>` (incluye `magento/informacion-de-orden`), `lead-times`, `cupones`, `orden-info`, `starkoms`, `lgcom[/popup]`, `seller-center-falabella`, `e-promoters`, `pim`, `solotodo`, `gato`, `registro-acciones`, `vpn`, `facturas`) más `content`, `service-worker`, `debug`, `popup`.
...
- **`wireAsyncRunLifecycle({ subscribeToRun, tickIfActive, abortActiveRun?, reconcileOnInit?, topFrameOnly?, log })`** — storage-driven async (Colocar TAGs, Starkoms, Seller Center, PIM, SoloTodo, Informacion de Orden). `tickIfActive(run)` recibe el run que llego por `storage.onChanged` (opcional; Informacion de Orden lo usa para detectar que otra pestaña reclamo la corrida). **Ojo:** el flag `claimed` no es atomico entre pestañas; si una feature puede correr en dos pestañas del mismo sitio, reclamar con token verificable (patron en `informacion_de_orden/content/flows/run.js`).
- **`wireReloadTickLifecycle({ runKey, tickIfActive, abortActiveRun?, delay=300, topFrameOnly=true, log })`** — tick-por-reload (Magento, Lead Times, Cupones; Facturas con `topFrameOnly:false` porque su pantalla vive en un iframe). **Ojo:** ese tick corre en el MISMO documento que acaba de pedir la navegación (el `storage.onChanged` llega en ~1 ms; el navegador tarda cientos). Si el flow marca un item "en curso", escribe y recién después navega, el tick lo ve "en curso" en la página vieja → usar el flag `navigating` (patrón en `magento/content/flows/run.js`).
...
| Registro de acciones | Graba clics, campos, teclas y navegación del usuario en cualquier sitio y lo exporta en Markdown para analizar el flujo con una IA. Otra feature puede grabar su corrida y anotar lo que hace (`TIPOS.EXTENSION`, `MESSAGES.ANOTAR`) | Port + SW + IndexedDB | `registro-acciones.md` |
| Facturas | Carga de facturas y notas de credito de comision en GEVS (Complex Voucher, Oracle OAF) desde el Invoice Master File (xlsx): cabecera, credito, lineas por BU + IVA, DFF, adjuntos, Save. La nota de credito es la misma pantalla con prefijo `CN` y todo en negativo (el signo viene del Excel; el driver no distingue tipos). **El Submit lo hace la persona** (la extension lo indica y lee la Reference). Bitacora opcional de la corrida (usuario + extension) via Registro de acciones. **Probada en GEVS real hasta Save + adjuntos** | tick-por-reload (top frame) + claim por pestana | `facturas.md` (+ `facturas-flujo-gevs.md`, `facturas-datos.md`) |
