...
**Búsqueda por SKU (`flows/search.js`):** `ensureStgTab` (click `#stg-tab` nativo si no tiene clase `active`) → `setInputValue(#productId, sku)` → `#search_sales_model_code.click()` (nativo, botón legacy con `onclick`) → **`waitForSearchToStart`** (gate anti-stale, ver quirk) → `waitFor(resolveResult().result !== 'pending')` (timeout `DEFAULTS.searchTimeoutMs`=15s) → si `found` y `specAssign` vacío, **re-lee `readSpecAssign(sku)`** hasta `DEFAULTS.specSettleMs`=2s (el link `spec-link` se puebla tras la fila). Devuelve `{ found, specAssign }`.

**Resolución del resultado (`parser.js#resolveResult`):** ámbito = pestaña `#stg` (fallback `document`). Devuelve `{ result, specAssign }`. `result='found'` si alguna fila `.tui-grid-rside-area ... tbody tr` tiene una celda `.tui-grid-cell-content` que matchea el SKU (== `Sales Model Code`, o `SKU (Product ID)` empieza por `SKU.`); `'not-found'` si la capa `.tui-grid-layer-state` está visible con texto "No data." y ninguna fila matchea; si no, `'pending'`. Matchear la fila por el SKU evita leer resultados de la búsqueda anterior (grillas stale). **`specAssign`** = contenido de la columna **"Spec Assign"** (`td[data-column-name="specAssignmentCode"]`, ej "Assigned") de la fila que matchea; se ubica por su `data-row-key` (la celda puede vivir en lside/rside), o null si vacía/no existe. `readSpecAssign(sku)` (exportada) relocaliza la fila y re-lee el spec (para el settle async).
...
- **Spec Assign async:** el `<a class="spec-link">` de "Spec Assign" se puebla un instante **después** de renderizar la fila. Leerlo al detectar `found` da `null` (celda vacía) → "—" en todos. Fix: si viene vacío, re-leer con `waitFor(readSpecAssign(sku))` hasta `specSettleMs`; vacío tras el tope = producto sin Spec Assign real.
