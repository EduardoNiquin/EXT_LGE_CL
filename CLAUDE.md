...
| Facturas | Carga de facturas de comision en GEVS (Complex Voucher, Oracle OAF) desde el Invoice Master File (xlsx): cabecera, credito, lineas por BU + IVA, DFF, adjuntos, Save. **El Submit lo hace la persona** (la extension lo indica y lee la Reference). Bitacora opcional de la corrida (usuario + extension) via Registro de acciones. **Probada en GEVS real hasta Save + adjuntos** | tick-por-reload (top frame) + claim por pestana | `facturas.md` (+ `facturas-flujo-gevs.md`, `facturas-datos.md`) |
